...
public class ProblemResult {
  ...
  /**
   * Enum: PRELIMINARY, FINAL. If type is PRELIMINARY then points can decrease (if, for example, solution will fail
   * during system test). Otherwise, party can only increase points for this problem by submitting better solutions.
   */
  SubmissionType type;

  /**
   * Integer. Number of seconds after the start of the contest before the submission, that brought maximal amount of
   * points for this problem.
   */
  int bestSubmissionTimeSeconds;

  public ProblemResult(double points, int penalty, int rejectedAttemptCount, SubmissionType type,
      int bestSubmissionTimeSeconds) {
    this.points = points;
